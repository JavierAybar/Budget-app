...
RSpec.describe Operation, type: :model do
  ...
  it 'is not valid without a name' do
    ...
    group = Group.create(user: user, name: 'Example Group', icon: 'example.png')
    operation = Operation.new(user: user, amount: 100, groups: [group])
    expect(operation).not_to be_valid
  ...
  it 'is not valid without an amount' do
    user = User.create(name: 'Javier', email: 'email@gmail.com', password: 'password')
    group = Group.create(user: user, name: 'Example Group', icon: 'example.png')
    operation = Operation.new(user: user, name: 'Example Operation', groups: [group])
    expect(operation).not_to be_valid
  ...
  it 'is not valid without a group' do
    user = User.create(name: 'Javier', email: 'email@gmail.com', password: 'password')
    operation = Operation.new(user: user, name: 'Example Operation', amount: 100)
    expect(operation).not_to be_valid
  ...
  it 'has a user associated' do
    user = User.create(name: 'Javier', email: 'email@gmail.com', password: 'password')
    group = Group.create(user: user, name: 'Example Group', icon: 'example.png')
    operation = Operation.new(user: user, name: 'Example Operation', amount: 100, groups: [group])
